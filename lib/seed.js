...
export const PROVIDERS = [
  // { id: "p_gymflow", name: "GymFlow Tirana", category: "fitness", city: "Tirana", emoji: "🏋️", logo: "/logos/gymflow.png", blurb: "Modern gyms in Blloku & Komuna e Parisit." },
  ...
  // { id: "p_skyspa", name: "Sky Spa Tirana", category: "wellness", city: "Tirana", emoji: "💆", logo: "/logos/skyspa.png", blurb: "Rooftop spa, sauna & massage." },
  ...
  // { id: "p_komiteti", name: "Komiteti Kafe-Muzeum", category: "food", city: "Tirana", emoji: "☕", logo: "/logos/komiteti.png", blurb: "Retro café, raki & local plates." },
  { id: "p_wolt", name: "Wolt Albania", category: "food", city: "Tirana", emoji: "🛵", logo: "/logos/wolt.png", blurb: "Food delivery credit, citywide." },
  // { id: "p_saranda", name: "Saranda Escapes", category: "travel", city: "Sarandë", emoji: "🏖️", logo: "/logos/saranda.png", blurb: "Riviera weekends & island day-trips." },
  // { id: "p_alps", name: "Albanian Alps Co.", category: "travel", city: "Theth", emoji: "🏔️", logo: "/logos/alps.png", blurb: "Guided hikes in Theth & Valbona." },
  // { id: "p_coursera", name: "SkillUp Academy", category: "learning", city: "Online", emoji: "🎓", logo: "/logos/skillup.png", blurb: "Online courses & certifications." },
  ...
  // { id: "p_hygeia", name: "Hygeia Hospital", category: "health", city: "Tirana", emoji: "🏥", logo: "/logos/hygeia.png", blurb: "Check-ups & specialist visits." },
  // { id: "p_smile", name: "Smile Dental Studio", category: "health", city: "Tirana", emoji: "🦷", logo: "/logos/smile.png", blurb: "Dental care & whitening." },
  ...
  // { id: "p_bolt", name: "Bolt", category: "transport", city: "Tirana", emoji: "🚕", logo: "/logos/bolt.png", blurb: "Rides & scooters across the city." },
];
...
export const OFFER_MAP = byId(OFFERS);
export const PROVIDER_MAP = byId(PROVIDERS);
export const PACKAGE_MAP = byId(PACKAGES);
...
export function packagePriceALL(pkg) {
  const raw = pkg.offerIds.reduce((s, oid) => s + (OFFER_MAP[oid]?.priceALL || 0), 0);
  // packages get a friendly 10% bundle discount
  return Math.round(raw * 0.9);
}
export function packageRawPriceALL(pkg) {
  return pkg.offerIds.reduce((s, oid) => s + (OFFER_MAP[oid]?.priceALL || 0), 0);
}

// Compact catalog string for the AI — keeps tokens low, ids stable.
export function catalogForAI() {
  const offers = OFFERS.map(
    (o) =>
      `${o.id} | ${o.title} | ${CATEGORY_MAP[o.category]?.en} | ${o.priceALL} ALL | ${PROVIDER_MAP[o.providerId]?.name} | tags:${o.tags.join(",")}`
  );
  const packages = PACKAGES.map(
    (p) => `${p.id} | PACKAGE "${p.title}" | ${packagePriceALL(p)} ALL | includes:${p.offerIds.join("+")}`
  );
