// Real Claude integration for PERX. Server-only (uses the API key).
...
function catalogBlock() {
  const { offers, packages } = catalogForAI();
...
// ---------------------------------------------------------------------------
export async function concierge({ history, budgetLeftALL, lang = "en", userName = "there" }) {
  const system = `You are the PERX Concierge — a warm, witty benefits buddy for employees in Albania. PERX is a workplace benefits marketplace: the company funds the budget, the employee just picks what they love, and payment goes straight to providers.
...
Only include a bundle when the user would benefit from combining 2-3 offers from different providers. Keep recommendIds tight and relevant to what they actually asked.`;
  ...
  return sanitizeRecommendation(json);
}

// ---------------------------------------------------------------------------
// 2) PERSONALIZED HOME RECOMMENDATIONS
// ---------------------------------------------------------------------------
export async function recommendForUser({ user, budgetLeftALL, ownedOfferIds = [], lang = "en" }) {
  const system = `You are PERX's personalization engine. Pick benefits an employee is most likely to love and explain why in one delightful line each.
...
Mix categories, respect budget, prefer variety over repetition.`;
  ...
  json.items = json.items.filter((it) => OFFER_MAP[it.id] || PACKAGE_MAP[it.id]).slice(0, 6);
  return json;
}
...
// ---------------------------------------------------------------------------
export async function buildBundle({ goal, budgetALL, lang = "en" }) {
  const system = `You are PERX's smart bundler. Compose ONE package of 2-4 offers from DIFFERENT providers that together fulfil the user's goal within budget (${budgetALL} ALL). ${
...
Only use offer ids (not package ids). Prefer offers from different providers. Stay within budget.`;
  ...
  json.offerIds = (json.offerIds || []).filter((id) => OFFER_MAP[id]).slice(0, 4);
  if (json.offerIds.length === 0) throw new Error("bundle: empty");
  json.priceALL = json.offerIds.reduce((s, id) => s + OFFER_MAP[id].priceALL, 0);
  return json;
}
...
export async function employerInsights({ stats, lang = "en" }) {
  const system = `You are PERX's analytics co-pilot for People Ops. Turn raw benefit-usage stats into 3 sharp, decision-ready insights. ${
...
Return ONLY JSON:
{
  ...
  "insights": [ { "emoji": "📈", "title": "short", "detail": "1 sentence with a number + a recommendation" }, x3 ]
}`;
  const text = await callClaude({
    system,
...
// ---------------------------------------------------------------------------
function sanitizeRecommendation(json) {
  json.recommendIds = (json.recommendIds || [])
    .filter((id) => OFFER_MAP[id] || PACKAGE_MAP[id])
    .slice(0, 4);
  if (json.bundle) {
    json.bundle.offerIds = (json.bundle.offerIds || []).filter((id) => OFFER_MAP[id]).slice(0, 4);
...
export function heuristicRecommend({ user, budgetLeftALL }) {
  const scored = OFFERS.filter((o) => o.priceALL <= budgetLeftALL)
    .map((o) => ({
      o,
      score:
...
export function heuristicConcierge({ text, budgetLeftALL }) {
  ...
  const picks = OFFERS.filter((o) => o.tags.includes(vibe) && o.priceALL <= budgetLeftALL).slice(0, 3);
  const pool = picks.length ? picks : OFFERS.filter((o) => o.priceALL <= budgetLeftALL).slice(0, 3);
  return {
...
export function heuristicInsights({ stats }) {
  ...
  return {
    headline: "Your benefits program is live and being used.",
    insights: [
      { emoji: "🔥", title: "Most-loved category", detail: `${top ? top[0] : "Wellness"} leads on spend — consider adding more options there.` },
      { emoji: "💸", title: "Budget utilisation", detail: `${stats.utilisationPct || 0}% of welfare budget is committed this cycle.` },
      { emoji: "✨", title: "Quick win", detail: "Promote a seasonal package to re-engage employees who haven't picked yet." },
    ],
  };
}
