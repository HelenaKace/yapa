...
function paymentRecords(items, processedAt = Date.now()) {
...
function seedOrder({ id, employeeId, items, note, status, createdAt, decidedAt = null }) {
  ...
  const payments = approved ? paymentRecords(normalized, decidedAt) : [];
  return {
    id,
    employeeId,
    items: normalized,
...
export function decideOrder({ orderId, decision }) {
  ...
  if (decision === "approve") {
    const payments = paymentRecords(order.items, decidedAt);
