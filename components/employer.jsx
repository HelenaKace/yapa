  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-28 pt-6">
      <div className="mb-6 overflow-hidden rounded-5xl grad-blue p-6 text-white shadow-pop">
        <Pill className="!bg-white/20 !text-white">{EMPLOYER.emoji} {EMPLOYER.name}</Pill>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        <Kpi label="Pending approvals" value={pending.length} emoji="⏳" />
        <Kpi label="Budget utilised" value={`${stats?.utilisationPct ?? 0}%`} emoji="💸" />
        <Kpi label="Active employees" value={`${stats?.activeEmployees ?? 0}/${EMPLOYEES.length}`} emoji="🧑‍🤝‍🧑" />
        <Kpi label="Total committed" value={<Money all={stats?.totalSpendALL || 0} />} emoji="📊" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                {decided.slice(0, 5).map((o) => {
                  ...
                  return (
                    <div key={o.id} className="card flex items-center justify-between p-3 text-sm">
                      <span>{emp?.emoji} {emp?.name} · {o.items.length} item{o.items.length > 1 ? "s" : ""}</span>
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${o.status === "approved" ? "grad-blue text-white" : "bg-perx-ink/10"}`}>
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-2">
                  {(insights?.insights || []).map((ins, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                      ...
                      <p className="text-sm font-semibold">{ins.emoji} {ins.title}</p>
          ...
          <div className="card p-5">
            <h3 className="mb-3 font-display text-lg font-semibold">👥 Team budgets</h3>
            <div className="space-y-3">
              {EMPLOYEES.map((e) => {
                ...
                return (
                  <div key={e.id}>
                    <div className="mb-1 flex items-center justify-between text-sm">
                      <span>{e.emoji} {e.name.split(" ")[0]}</span>
    ...
    <motion.div layout className="card p-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          <div className="grid h-10 w-10 place-items-center rounded-2xl grad-grape text-lg">{emp?.emoji}</div>
          <div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {order.items.map((it) => {
          ...
          return <span key={it.id} className="rounded-full bg-white/80 px-3 py-1 text-xs font-medium border border-white">{emoji} {title}</span>;
        })}
      </div>
      <div className="mt-2 text-xs font-semibold text-perx-blue">🔗 will pay {providers.size} provider{providers.size > 1 ? "s" : ""} directly</div>
      <div className="mt-3 flex gap-2">
        <button onClick={() => decide(order.id, "approve")} className="pop-btn flex-1 grad-blue py-2.5 text-sm font-semibold text-white shadow-pop-sm">
          ✅ Approve & pay
        </button>
